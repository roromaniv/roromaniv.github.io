    ...
    json.onload = function () {
      ...
      for (let i = 0; i < content["facts"].length; i++) {
        document.getElementById("facts").innerHTML +=
          '<div class="fact column"><h3 class="mbs t-black">{{VALUE}}</h3><p>{{FACT}}</p></div>'
            .replace("{{FACT}}", content.facts[i]["fact"])
            .replace("{{VALUE}}", content.facts[i]["value"]);
      }
      document.getElementsByClassName("button")[0].href = content.link;
      document.getElementsByClassName("button")[0].innerHTML +=
        content.buttonTitle;

      //PREVIEWS
      document.getElementsByClassName("preview")[0].style.backgroundImage =
        "url('" + content.images[0].link + "')";
      document.getElementsByClassName("preview")[1].style.backgroundImage =
        "url('" + content.images[1].link + "')";
      document.getElementsByClassName("preview")[2].style.backgroundImage =
        "url('" + content.images[2].link + "')";
      document.getElementById("moreimages").innerText =
      ...
      for (let i = 0; i < content.images.length; i++) {
        images +=
          '["' +
          content.images[i].link +
          '","' +
          content.images[i].name +
          '","' +
          content.images[i].description +
          '"],';
      }
